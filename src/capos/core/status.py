"""Pipeline and asset status enums."""

from __future__ import annotations

from enum import StrEnum


class StageStatus(StrEnum):
    """Explicit status for every production stage and asset."""

    DRAFT = "DRAFT"
    READY = "READY"
    GENERATING = "GENERATING"
    GENERATED = "GENERATED"
    QA_FAILED = "QA_FAILED"
    QA_PASSED = "QA_PASSED"
    REQUIRES_REVIEW = "REQUIRES_REVIEW"
    APPROVED = "APPROVED"
    REJECTED = "REJECTED"
    SUPERSEDED = "SUPERSEDED"
    EXPORTED = "EXPORTED"
    LOCKED = "LOCKED"
    MISSING = "MISSING"
    PENDING = "PENDING"
    REFUSED = "REFUSED"  # provider safety refusal recorded


class QAResultStatus(StrEnum):
    """Honest QA outcome — never invent PASS without inspection."""

    NOT_CHECKED = "NOT_CHECKED"
    CHECKED = "CHECKED"
    PASS = "PASS"
    FAIL = "FAIL"
    REQUIRES_HUMAN_REVIEW = "REQUIRES_HUMAN_REVIEW"


class FrameType(StrEnum):
    MASTER_REFERENCE = "MASTER_REFERENCE"
    KEYFRAME = "KEYFRAME"
    TRANSITION = "TRANSITION"
    DIALOGUE = "DIALOGUE"
    REACTION = "REACTION"
    ESTABLISHING = "ESTABLISHING"
    COVER = "COVER"
    THUMBNAIL = "THUMBNAIL"


class PropState(StrEnum):
    CLOSED = "CLOSED"
    OPEN = "OPEN"
    LID_BESIDE_JAR = "LID_BESIDE_JAR"
    OPEN_LID_RIGHT = "OPEN_LID_RIGHT"
    OPEN_LID_LEFT = "OPEN_LID_LEFT"
    EMPTY = "EMPTY"
    PARTIALLY_FULL = "PARTIALLY_FULL"
    CUSTOM = "CUSTOM"


class CanonicalAssetType(StrEnum):
    CHARACTER_MASTER = "CHARACTER_MASTER"
    CHARACTER_TURNAROUND = "CHARACTER_TURNAROUND"
    CHARACTER_EXPRESSION = "CHARACTER_EXPRESSION"
    CHARACTER_POSE = "CHARACTER_POSE"
    OUTFIT_MASTER = "OUTFIT_MASTER"
    LOCATION_MASTER = "LOCATION_MASTER"
    LOCATION_CAMERA = "LOCATION_CAMERA"
    LOCATION_PROP_LAYOUT = "LOCATION_PROP_LAYOUT"
    PROP_MASTER = "PROP_MASTER"
    PROP_STATE = "PROP_STATE"
    STYLE_MASTER = "STYLE_MASTER"
    EPISODE_REFERENCE = "EPISODE_REFERENCE"
    GOLDEN_FRAME = "GOLDEN_FRAME"


class CanonStatus(StrEnum):
    """Canonical asset lifecycle — only APPROVED may lock production."""

    DRAFT = "DRAFT"
    CANDIDATE = "CANDIDATE"
    REVIEW_REQUIRED = "REVIEW_REQUIRED"
    AWAITING_HUMAN_SELECTION = "AWAITING_HUMAN_SELECTION"
    AWAITING_STYLE_REFERENCE_IMPORT = "AWAITING_STYLE_REFERENCE_IMPORT"
    AWAITING_HUMAN_STYLE_RECOVERY_REVIEW = "AWAITING_HUMAN_STYLE_RECOVERY_REVIEW"
    BLOCKED_NO_PROVIDER = "BLOCKED_NO_PROVIDER"
    APPROVED = "APPROVED"
    REJECTED = "REJECTED"
    HUMAN_REJECTED_STYLE_DRIFT = "HUMAN_REJECTED_STYLE_DRIFT"
    SUPERSEDED = "SUPERSEDED"
    REFERENCE_REQUIRED = "REFERENCE_REQUIRED"


class VisualReferenceType(StrEnum):
    STYLE_REFERENCE = "STYLE_REFERENCE"
    CHARACTER_REFERENCE = "CHARACTER_REFERENCE"
    LOCATION_REFERENCE = "LOCATION_REFERENCE"
    PROP_REFERENCE = "PROP_REFERENCE"
    GOLDEN_FRAME_REFERENCE = "GOLDEN_FRAME_REFERENCE"


class CanonStep(StrEnum):
    """Dependency-ordered canon creation steps."""

    STYLE_MASTER = "STYLE_MASTER"
    STYLE_RECOVERY = "STYLE_RECOVERY"
    LIKKLE_JAY_MASTER = "LIKKLE_JAY_MASTER"
    AUNTIE_BEV_MASTER = "AUNTIE_BEV_MASTER"
    CHARACTER_TURNAROUNDS = "CHARACTER_TURNAROUNDS"
    CHARACTER_EXPRESSIONS = "CHARACTER_EXPRESSIONS"
    LOCATION_MASTERS = "LOCATION_MASTERS"
    PROP_MASTERS = "PROP_MASTERS"
    GOLDEN_FRAMES = "GOLDEN_FRAMES"


CANON_STEP_ORDER: tuple[CanonStep, ...] = tuple(CanonStep)


class ProviderCapability(StrEnum):
    TEXT_TO_IMAGE = "TEXT_TO_IMAGE"
    IMAGE_TO_IMAGE = "IMAGE_TO_IMAGE"
    REFERENCE_IMAGE = "REFERENCE_IMAGE"
    INPAINT = "INPAINT"
    OUTPAINT = "OUTPAINT"
    CONTROL_IMAGE = "CONTROL_IMAGE"
    SEED = "SEED"
    NEGATIVE_PROMPT = "NEGATIVE_PROMPT"


class ProviderAvailability(StrEnum):
    AVAILABLE = "AVAILABLE"
    NOT_CONFIGURED = "NOT_CONFIGURED"
    UNAVAILABLE = "UNAVAILABLE"
    DEGRADED = "DEGRADED"


class ComedyBeat(StrEnum):
    SETUP = "SETUP"
    TEMPTATION = "TEMPTATION"
    MISUNDERSTANDING = "MISUNDERSTANDING"
    ESCALATION = "ESCALATION"
    DISCOVERY = "DISCOVERY"
    REACTION = "REACTION"
    PUNCHLINE = "PUNCHLINE"


class ProductionStage(StrEnum):
    IDEA = "IDEA"
    EPISODE_BRIEF = "EPISODE_BRIEF"
    SCRIPT = "SCRIPT"
    DIALOGUE_REVIEW = "DIALOGUE_REVIEW"
    STORYBOARD = "STORYBOARD"
    CONTINUITY_PLAN = "CONTINUITY_PLAN"
    REFERENCE_RESOLUTION = "REFERENCE_RESOLUTION"
    KEYFRAME_GENERATION = "KEYFRAME_GENERATION"
    FRAME_QA = "FRAME_QA"
    TRANSITION_GENERATION = "TRANSITION_GENERATION"
    ANIMATION_PLAN = "ANIMATION_PLAN"
    ANIMATION_ASSEMBLY = "ANIMATION_ASSEMBLY"
    VOICE = "VOICE"
    SFX = "SFX"
    MUSIC = "MUSIC"
    SUBTITLES = "SUBTITLES"
    FINAL_CONTINUITY_QA = "FINAL_CONTINUITY_QA"
    COVER_THUMBNAIL = "COVER_THUMBNAIL"
    FINAL_MP4 = "FINAL_MP4"
    HUMAN_APPROVAL = "HUMAN_APPROVAL"
    PUBLISHABLE_EXPORT = "PUBLISHABLE_EXPORT"


# Ordered pipeline for workflows
PIPELINE_ORDER: tuple[ProductionStage, ...] = tuple(ProductionStage)
